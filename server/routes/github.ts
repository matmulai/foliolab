...
router.post("/api/fetch-repos", async (req, res) => {
  ...
  try {
    ...
    console.log(
      `Successfully authenticated user: ${githubUser.username}, found ${repos.length} repositories`,
    );
    ...
    if (error instanceof Error) {
      if (error.message.includes("GitHub OAuth error")) {
        return res.status(400).json({
          ...
          suggestion:
            "Please try signing in again. If the problem persists, the authorization code may have expired.",
        });
      }
    }
    ...
    res.status(500).json({
      ...
      suggestion:
        "Please try again. If the problem persists, try clearing your browser cache and signing in again.",
    });
  }
});
