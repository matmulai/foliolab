...
export default function RepoSelect() {
  ...
  const { mutate: toggleRepo, isPending: isToggling } = useMutation({
    mutationFn: async ({ id, selected }: { id: number; selected: boolean }) => {
      ...
      // Update cache
      queryClient.setQueryData<{ repositories: Repository[] }>(
        ["/api/repositories"],
        {
  ...
  const owners = useMemo(() => {
    ...
    const ownerMap = new Map<
      string,
      { login: string; type: string; avatarUrl: string | null }
    >();
  ...
  const selectedRepos =
    data?.repositories.filter((repo) => repo.selected) || [];
  ...
  const allSelected =
    paginatedRepos.length > 0 && paginatedRepos.every((repo) => repo.selected);
      ...
      for (let i = 0; i < selectedRepos.length; i++) {
        ...
        if (repo.id) {
          ...
          const res = await apiRequest(
            "POST",
            `/api/repositories/${repo.id}/analyze`,
            {
            ...
            (old) => {
              ...
              return {
                repositories: old.repositories.map((r) =>
                  r.id === data.repository.id
                    ? { ...r, summary: data.repository.summary }
                    : r,
                ),
              };
            },
